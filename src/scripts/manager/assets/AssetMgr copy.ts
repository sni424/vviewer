...
export class AssetMgr {
  static downloadAll() {
    // for every cache key/value, download file name [key] = value
    const files = Array.from(this.cache.keys());
    const allCount = files.length;
    let binaryCount = 0;
    for (let i = 0; i < allCount; i++) {
      console.log(`Downoad ${i + 1}/${allCount}`);
      const file = files[i];
      ...
      if (
        ...
        a.href = URL.createObjectURL(new Blob([cached.data]));
        a.download = file + '.bin';
        a.click();
    ...
    console.log('All files downloaded, Binary count :', binaryCount);
  }

  static cache: Map<
    FileID | AssetCacheType, // AssetCacheType도 받는 이유 : 객체주소를 가지고 있는지 MapHash로 O(1)로 확인하기 위해서
    | {
        id: FileID;
        state: 'loading';
        data: Promise<AssetCacheType | VLoadable>;
      }
    | { id: FileID; state: 'loaded'; data: AssetCacheType | VLoadable }
  > = new Map();
  ...
  static set(rawData: AssetCacheType, id?: string): FileID {
    ...
    cacher.set(hash, cacheData); // id : cacheData
    // cacher.set(rawData, cacheData); // rawData주소 : cacheData
    return hash;
  }

  static has(id: FileID): boolean;
  static has(rawData: AssetCacheType): boolean;
  static has(idOrRawData: FileID | AssetCacheType): boolean {
    return this.cache.has(idOrRawData);
  }
