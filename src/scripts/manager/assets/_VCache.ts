...
export type _CachePayload<T = any> = {
...
export type _PayloadValue<T = any> = _CachePayload<T>[keyof _CachePayload<T>];

export type _CachePayloadTarget<T = any> = {
  destination: keyof _CachePayload<T>;
  from: File | VFile | VRemoteFile | string | any; // 무엇으로부터 promise를 만드는지. 무엇이 됐든 loadingQueue에 추가하기 전에 이미 이 from이 있는지를 검사하기 위한 일종의 id임
  promise?: Promise<_CachePayload<T>[keyof _CachePayload<T>]>;
  data?: _CachePayload<T>[keyof _CachePayload<T>];
};
export type _Promisable<T = any> = T | _CachePayloadTarget<T>;

export type _CacheValue<T = any> = {
  id: string;
  hash: string; // 첫 시작시는 id == hash이지만 데이터가 변화됨에 따라 hash가 바뀜
  payload: T;
  loadingQueue: _CachePayloadTarget[];
};

class _CacheValueHasher {
  static objectHashMap = new WeakMap<object, string>();
  static objectHashCounter = 0;

  static payload(payload: _CachePayload): string {
    const keys = [
      'file',
      'inputBuffer',
      // 'obj',
...
let _cacheCount = 1;
export default class _VCache {
  static isLoading(value: _CacheValue<any>) {
    return value.loadingQueue.length > 0;
  }

  constructor(public name: string = 'Cache' + _cacheCount++) {}

  cache: Map<FileID, _CacheValue<_CachePayload>> = new Map();
  ...
  _upsertFile<T>(cacheValue: _CacheValue<_CachePayload<T>>, value?: File) {
  ...
  _upsertVFile<T>(cacheValue: _CacheValue<_CachePayload<T>>, value?: VFile) {
  ...
  _upsertResult<T>(cacheValue: _CacheValue<_CachePayload<T>>, value?: T) {
  ...
  _upsert<T = any>(
    ...
    cacheValue.hash = _CacheValueHasher.payload(payload);
  ...
  set<T = any>(id: string, ...value: _Promisable<_PayloadValue<T>>[]): this;
  ...
  set<T = any>(
    ...
    // VFile인 경우 대응
    let id: string;
    let value: _Promisable<_PayloadValue<T>>;
    if (isVFile(idCandidate) || isVRemoteFile(idCandidate)) {
      ...
      value = idCandidate as _Promisable<_PayloadValue<T>>;
    } else {
      id = idCandidate as string;
      //  이 경우 반드시 valueCandidate의 길이가 1이어야 함
      value = valueCandidate[0] as _Promisable<_PayloadValue<T>>;
    }
    ...
    const targetValue: _CacheValue<_CachePayload<T>> = hasCache
      ? cached
      : {
          id,
          hash: id,
    ...
    if (this.isCachePayloadTarget(value)) {
      const target = value as _CachePayloadTarget<T>;
  ...
  getId(query: _PayloadValue): string | undefined {
  ...
  get<T = any>(id: FileID): _CacheValue<_CachePayload<T>> | undefined;
  get<T = any>(buffer: ArrayBuffer): _CacheValue<_CachePayload<T>> | undefined;
  get<T = any>(file: File): _CacheValue<_CachePayload<T>> | undefined;
  get<T = any>(vfile: VFile): _CacheValue<_CachePayload<T>> | undefined;
  get<T = any>(
    vremotefile: VRemoteFile,
  ): _CacheValue<_CachePayload<T>> | undefined;
  get<T = any>(result: T): _CacheValue<_CachePayload<T>> | undefined;
  get<T = any>(
