...
const _EnvController = () => {
    ...
    return <div style={{ marginTop: 4, display: "flex", flexDirection: "column" }}>
        환경맵
        ...
        {env.select === "custom" && <>
            <div style={{
                ...
                가져온 환경맵 :
                <select value={env.url ?? "-"} onChange={e => {
                    setEnv({
                        ...env,
                        url: e.target.value
                    })
                    ...
                    {envs.map(envInfo => {
                        return <option key={`file-${envInfo.fileUrl}`} value={envInfo.fileUrl}>{envInfo.filename}</option>
                    })}
                </select>
