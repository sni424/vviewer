...
export const ViewportRenderer = (view: View) => ({ children }: {
...
export const Viewport = (view: View) => ({ children, onCreated, ...rest }: {
    children?: React.ReactNode,
...
export const useGetThreeExports = (view: View = View.Shared) => {
    ...
    const [threeExports, setThreeExports] = useState<RootState | undefined>(exists);
...
export const ViewController = (view: View) => ({ children, hide }: {
    children?: React.ReactNode,
    hide: () => void
}) => {
    ...
    return <div style={{
        ...
        <div>
            ...
            }}>그리드 {getOption.grid ? "끄기" : "켜기"}</button>
        </div>
        {children}
    </div>
}
