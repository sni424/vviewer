...
function Gizmo() {
    ...
    const boxMesh = useMemo(() => {
        ...
        return () => < mesh ref={meshRef}>
            <Box args={boundingBox.getSize(new THREE.Vector3()).toArray()} position={boundingBox.getCenter(new THREE.Vector3()).toArray()} >
    ...
    return (
        <>
            {boxMesh}
            <TransformControls position={meshRef.current?.position} name="TransformControl" object={meshRef.current!} onChange={(e) => {
                console.log(e);
            }}></TransformControls>
        </>
    )
