...
const Checkbox = (props: {
  label: string;
  propKey: keyof GLProps;
  onChange?: (value: boolean) => any;
}) => {
  const { gl, updateGl } = useGlobalREnderOption();
  const { label, propKey, onChange } = props;
  return (
    <div style={{ fontSize: 11, paddingLeft: 6 }}>
      <span>{label}</span>
      <input
        type="checkbox"
        checked={gl[propKey] as boolean | undefined}
        onChange={e => {
    ...
    <div style={{ fontSize: 11, paddingLeft: 6 }}>
      ...
      >
        {options.map(option => {
          return (
            <option
              key={`global-dropdown-${propKey}-${option.name}`}
              value={option.value}
            >
              {option.name}
            </option>
