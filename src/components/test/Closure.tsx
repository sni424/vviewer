...
function _isPointInsidePolygon(point: Point, polygon: Point[]): boolean {
  ...
  for (let i = 0; i < polygon.length; i++) {
    ...
    if (vertex1.y > y !== vertex2.y > y) {
      const intersectX =
        ((vertex2.x - vertex1.x) * (y - vertex1.y)) / (vertex2.y - vertex1.y) +
        vertex1.x;
      if (x < intersectX) {
        count++;
      }
    ...
    <div>
      {sampleTests.map((test, i) => {
        return (
          <div>
            {i + 1}.{' '}
            {_isPointInsidePolygon(test, samplePoints) ? 'true' : 'false'}
          </div>
        );
      })}
    </div>
...
const TheScene = () => {
  ...
  return (
    <>
      ...
      <Environment
        files={
          'https://vra-configurator-dev.s3.ap-northeast-2.amazonaws.com/models/dancing_hall_1k.hdr'
        }
      ></Environment>
    </>
  );
...
function isPointInsidePolygons(
  point: Point2D,
  polygons: Point2D[][],
): number | undefined {
  ...
  return (
    <div
      style={{
        ...
        right: 10,
        padding: 8,
        backgroundColor: isInside ? 'green' : 'red',
        color: 'white',
      }}
    >
      Is Inside : {isInside ? 'O' : 'X'}
    </div>
  );
};
    ...
    <div style={{ width: '100dvw', height: '100dvh', position: 'relative' }}>
      ...
      <div
        style={{
          ...
          transform: `translate(${canvasX}px, 0px)`,
          overflow: 'hidden',
        }}
      >
        {/* <Canvas>
